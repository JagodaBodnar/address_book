...
public class AddressBookRepository {
    ...
    public AddressBook findByStreetName(String street) throws AddressesBookNotFoundException {
        return addresses.stream().filter(address -> address.getId().equals(street)).findAny().orElseThrow(AddressesBookNotFoundException::new);
    }
    public AddressBook findByName(String name) throws AddressesBookNotFoundException {
        return addresses.stream().filter(address -> address.getId().equals(name)).findAny().orElseThrow(AddressesBookNotFoundException::new);
    }
